...
func TestVisualModeYankLeavesEntriesInPlaceAndFillsRegister(t *testing.T) {
	m := visualFixtureModel(t)
	m.cursor = findRow(t, m, "First")
	m = sendKey(m, "V")
	m = sendKey(m, "j")
	...
	if m.mode != normalMode {
		t.Errorf("mode after visual y = %v, want normalMode", m.mode)
	}
	if len(m.rows) != rowsBefore {
		t.Errorf("rows after visual y = %d, want %d (nothing removed)", len(m.rows), rowsBefore)
	}
	for _, title := range []string{"First", "Second", "Child of second", "Third"} {
		if rowIndex(m, title) < 0 {
			t.Errorf("%q should still be present after visual y", title)
		}
	}

	if len(m.register) != 2 {
		t.Fatalf("register after visual y = %v, want 2 entries (First, Second)", m.register)
	}
	if m.register[0].Title != "First" || m.register[1].Title != "Second" {
		t.Errorf("register titles = %q, %q, want First, Second (top-to-bottom order)", m.register[0].Title, m.register[1].Title)
	}
	// The register must hold independent clones, not the live headlines.
	if m.register[0] == findHeadlineByTitle(t, m, "First") {
		t.Errorf("register holds the live headline pointer, want an independent clone")
	}
