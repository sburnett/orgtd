...
func TestMarkedAndDirtyRowShowsBothIndicators(t *testing.T) {
	...
	// Column 1 is the :format-links lock column (see lockColumn), column
	// 2 the "attached to a meeting" column (see meetingColumn) — both
	// blank here since neither applies.
	if line[1] != ' ' {
		t.Errorf("row = %q, want the lock column blank in column 1", string(line))
	}
	if line[2] != ' ' {
		t.Errorf("row = %q, want the meeting column blank in column 2", string(line))
	}
	if line[3] != '+' {
		t.Errorf("row = %q, want the dirty marker in column 3 alongside the mark", string(line))
	}
}
